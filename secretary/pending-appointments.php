<?php
    ...
    if (isset($_POST['del_app'])) {
        $del_id = $_POST['del_id'];
        $result = mysqli_query($con, "SELECT * FROM appointments WHERE appointments.app_id = '$del_id'");
        ...
        if ($query = mysqli_query($con, "DELETE FROM appointments WHERE appointments.app_id = '$del_id'")){
            $transac_mes = $sec_name . ' has declined '.$full_name.' appointment.';
    ...
    if (isset($_POST['con_app'])) {
        $con_id = $_POST['con_id'];
        $result = mysqli_query($con, "SELECT * FROM appointments WHERE appointments.app_id = '$con_id'");
        $rows = mysqli_fetch_assoc($result);
        $patients_id = $rows['patients_id'];
        $result1 = mysqli_query($con, "SELECT * FROM users WHERE users.patients_id = '$patients_id'");
        $rows1 = mysqli_fetch_assoc($result1);
        ...
        if ($query = mysqli_query($con, "UPDATE appointments SET status = '1' WHERE appointments.app_id = '$con_id'")){
            $transac_mes = $sec_name . ' has confirmed '.$full_name.' appointment.';
            $query = mysqli_query($con, "INSERT INTO transacs (transac_datetime, transac_mes, transac_user) VALUES (current_timestamp(), '$transac_mes', '$sec_name')");
        }
    }
?>
                ...
                <div class="card bg-default shadow">
                    ...
                    <div class="table-responsive">
                        <table class="table align-items-center table-dark table-flush">
                            ...
                            <tbody>
                            <?php
                            $query = mysqli_query($con,"SELECT * FROM appointments WHERE appointments.status = '0' AND appointments.sec_id = '$sec_id'");
